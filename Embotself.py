...
RfuBot=[lineMID]
Family=["u080454625a082b5818071bde115e45af",lineMID]
...
settings = {
    ...
    "pname": {},
    "pro_name": {},
    "message":"บัญชีนี้ได้การป้องกันโดย เอ็มBotSelf ระบบได้ทำการบล็อคคุณอัตโนมัติ",
...
def myhelp():
    myHelp = "____ 🔛เอ็ม Bot Self🔛_____ " + "\n" \
                  "°•About" + "\n" + \
                  "°•คำสั่งโซเชียล" + "\n" + \
                  "°•คำสั่งคิก" + "\n" + \
                  "°•คำสั่งกลุ่ม" + "\n" + \
                    ...
                    try:
                        ...
                        if strnum == "off":
                                ...
                                if settings["lang"] == "JP":
                                    ...
                                    line.sendText(msg.to,strnum + "ปฏิเสธกลุ่มที่มีคำเชิญอัตโนมัติ")
                    except:
                        if settings["lang"] == "JP":
...
#==============================================================================#
                    ...
                    sendMessageWithMention(to, lineMID)
                    line.sendContact(to, "u080454625a082b5818071bde115e45af")
                elif text.lower() == 'มิด':
...
#==============================================================================#
                    ...
                    timeNow = datetime.now(tz=tz)
                    day = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday","Friday", "Saturday"]
                    hari = ["อาทิตย์", "จะนทร์", "อังคาร", "พุธ", "พฤหัส", "ศุกร์", "เสาร์"]
                    bulan = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"]
                    hr = timeNow.strftime("%A")
                    bln = timeNow.strftime("%m")
                    for i in range(len(day)):
...
#==============================================================================#   
                    ...
                    line.sendMessage(receiver, 'หยุดการทำงานเอ็มบอทSelf เรียบร้อยแล้ว')
                    print ("Selfbot Off")
                    exit(1)
...
#==============================================================================#
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                ...
                if msg.text in ["Me","me",".me",".Me","คท","/me"]:
                    line.sendText(msg.to,"เชคทำมัยหรออ!!!")
                ...
                if msg.text in ["runtime","Runtime","/uptime","ออน",".uptime"]:
                    line.sendText(msg.to,"นานยัง")							  
                if msg.text in [".มอง"]:
                    line.sendText(msg.to,"มองรายครับ")	  
                if msg.text in ["555","5555","55555"]:
                    line.sendText(msg.to,"เอ็มแนะนำว่า ไปหาหมอบ้างน่ะ-.-")	
        ...
        if op.type == 17:
        	...
        	line.sendMessage(op.param1, "สวัสดีครับ {}, Welcome to Group {}\nยินดีต้อนรับครับ・ω・".format(str(dan.displayName),str(tgb.name)))
        	line.sendContact(op.param1, op.param2)
        	line.sendImageWithURL(op.param1, "http://dl.profile.line-cdn.net{}".format(dan.picturePath))
        if op.type == 15:
        	dan = line.getContact(op.param2)
        	tgb = line.getGroup(op.param1)
        	line.sendMessage(op.param1, "อุ้ย!! {}, ได้ออกจากกลุ่ม {} \nรีบไปไหนเอ็มยังไม่ได้จีบเลย".format(str(dan.displayName),str(tgb.name)))
        	line.sendContact(op.param1, op.param2)
        if op.type == 55:
            try:
